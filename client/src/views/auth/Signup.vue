...
const welcomeMessage = computed(() => !localStorage?.lastUserName ? 'Login' : localStorage?.lastUserName + ', Welcome back')


const signup = () => {
    validateEmail()
    validatePassword()
    if (!error.value.password && !error.value.email) { //  Fields have been checked for accuracy
            ...
            <div class="flex-1  md:w-1/2 lg:w-1/3 mx-auto rounded-md drop-shadow-xl bg-white py-6 px-6 ">
                ...
                <div class="relative my-4">
                    ...
                    <label for="email" :class="{ 'text-red-600 ': error.email }"
                        class="absolute  hover:cursor-text  duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] left-2.5  peer-focus:dark:text-black text-muted peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-3">E-mail
                        or Phone
                    </label>
                ...
                <p class="text-center">New to LinkedIn? <router-link to="/signin"
                        class="text-primary hover:underline font-bold hover:cursor-pointer">Join Now</router-link></p>
            </div>
