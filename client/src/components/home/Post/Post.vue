    ...
    <div class="border pt-3  pb-1 px-4  text-sm bg-white rounded-lg">
        ...
        <div class="flex justify-between items-center mt-2 text-muted  text-xs">
            <div class="flex space-x-1">
                ...
                <span v-else>Be the first to like this.</span>
            </div>
            <p v-if="props.post.comments.length > 0">{{ props.post.comments.length }} comment</p>
        ...
        <ul class="flex items-center justify-between mx-auto  font-semibold">
            ...
            <li class="group">
                <button @click="toggleComment"
                    ...
                    <p class="hidden sm:block">Comment</p>
                </button>
            </li>
            <li class="group ">
                <button
        ...
        <div v-if="openCommentMenu" class="flex flex-col">
            ...
            <div :class="{ 'hidden': !hideCommentBtn }" class="pl-12 mt-[-5px] mb-1">
                <button @click="addComment"
                    class="bg-primary rounded-full px-3 py-1 font-semibold text-sm text-white">Publish</button>
            </div>
            <p v-if="props?.post?.comments?.length > 0" class="font-semibold text-muted flex justify-start items-center">Most
                relating to
